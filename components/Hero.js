...
export default function Hero({ config: serverConfig = {} }) {
  ...
  useEffect(() => {
    // Always fetch client-side — server is blocked on this machine
    // This runs in the browser where Airtable is reachable
    clientGetSiteConfig().then(c => {
      if (Object.keys(c).length > 0) setConfig(c);
    });
  }, []);
    ...
    <section style={{
      ...
      {imageUrl && (
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <img
            src={imageUrl}
            alt="Waren Odhiambo — Operational Infrastructure Builder"
